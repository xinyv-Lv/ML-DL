...
CNN 通过局部卷积高效提取模式，适合短距离依赖，但难以捕捉长程关系；

RNN 能理论上建模任意长序列，但串行计算导致训练慢，且存在梯度消失问题；

（LSTM 引入门控机制和细胞状态，用加法更新主信息流，有效缓解梯度消失；GRU 则将遗忘门和输入门合并为更新门，结构更简洁，训练更快。）

Self-Attention 则通过全局相似度计算，一步到位建立任意位置间的联系，完全并行，成为当前主流。
